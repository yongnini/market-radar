...
- `Real export`：来自真实数据导出，有采集日期，但不是实时 API。
...
项目也支持真实的 `Offline Reddit Export`。这是一份用本地 `rdt-cli` 抓取真实 Reddit 帖子后固化进 API 的离线数据，不依赖 Vercel 线上 OAuth。页面会把它标注为 `Real export`，并在 Reddit 列表中展示可点击的真实帖子记录。

`Offline Reddit VOC Snapshot` 是基于同一批真实 Reddit 导出帖子整理出的消费者洞察层。它适合在作品集中展示 Voice-of-Customer 分析能力，但不应被描述为实时 Reddit API 数据。
